...
export const getUserRentals = async (req: Request, res: Response, next: NextFunction)=>{
    try{
        const user = req.user;
        if(!user){
        ...
        if(userRentals.length === 0){
            return res.status(200).json({ success: true, message: "No rental clothes data exits", userRentals })
        }
...
export const rentCloth = async (req: Request, res: Response, next: NextFunction)=>{
    try{
        const user = req.user;
        if(!user){
            return next(new AppError("user doesn't exits, please login", 404));
        ...
        if(cloth.available === false){
            return next(new AppError("cloth is not available for rent, you can rent when it will available", 404))
        }
...
export const returnRentals = async (req: Request, res: Response, next: NextFunction)=>{
    try{
        const user = req.user;
        if(!user){
            return next(new AppError("user doesn't exits, please login", 404));
...
export const rentalsHistory = async (req: Request, res: Response, next: NextFunction)=>{
    try{
        const user = req.user;
        if(!user){
            return next(new AppError("user doesn't exits, please login", 404));
